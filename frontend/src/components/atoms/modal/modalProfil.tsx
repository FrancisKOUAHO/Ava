...
const ModalProfil = ({
      ...
      case 'prenom':
        return (
          <InputProfil
            ...
            name="text"
            placeholder="Prénom"
            className="border border-solid h-12 w-full rounded-md pl-2"
          />
        )
      case 'nom':
        return (
            ...
            <div className="relative w-full">
              ...
              <label className="block w-full h-12 border border-solid rounded-md pl-2 flex items-center justify-center text-gray-500 bg-white cursor-pointer">
                <span className="w-full text-center">
                  {imagePreviewUrl ? 'File selected' : 'No file chosen'}
                </span>
              </label>
            </div>
            {imagePreviewUrl && (
  ...
  return (
    <Transition.Root show={openModalProfil}>
      <Dialog
        className="flex z-50 items-center justify-center h-full w-full fixed top-0 left-0 pointer-events-none transition-all duration-200 ease-out"
            ...
            >
              <Dialog.Panel className="pl-[1.25rem] pr-[1.25rem] pb-[1.25rem] flex flex-col w-full  ">
                <div className=" flex w-full justify-end">
                  <CircleX
                    className="hover:text-[#0075eb] cursor-pointer"
                  ...
                  <div>
                    ...
                    {inputs.map((input, index) => (
                      <div key={index} className="flex flex-col mb-4">
                        <label className="text-left text-[1rem] font-[700] mb-[0.25rem] text-[25385E]">
                          {input.label}
                        </label>
                    ...
                    <div className="w-full flex justify-end mt-6">
                      <button
                        type="submit"
                        className="h-[36px] min-w-[64px] px-[16px] bg-[#0075eb] text-white text-[0.875rem] font-[500] rounded-md"
                      >
                        Modifier
                      </button>
                    </div>
