...
const EditInvoice: FunctionComponent<EditInvoiceProps> = ({ invoiceId }) => {
  ...
  useEffect(() => {
    if (invoiceDetails) {
      ...
      setBankName(invoiceDetails.bank)
      setIban(invoiceDetails.iban)
      setBic(invoiceDetails.bic)
  ...
  const handleSendInvoice = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response) {
        toast.error('CreateInvoice not sent', {
          position: 'top-right',
        })
        throw new Error('unknown error')
      }

      toast.success('CreateInvoice sent successfully', {
        position: 'top-right',
      })
    } catch (error) {
